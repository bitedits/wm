...
\title{Erlang OS.1: Bytecode Virtualized POSIX Systems}
...
\begin{abstract}
We present the \textbf{Erlang OS} systems architecture that reinterprets the traditional POSIX process model through the lens of the Erlang/OTP virtual machine.
...
applicability to specialised, high-integrity, and research-oriented POSIX systems.
\end{abstract}
...
Rather, we argue that it constitutes a coherent substrate for specialised systems---appliances, high-integrity desktops, educational platforms, and research vehicles---in which the benefits of a uniform, supervised, bytecode-verified execution environment outweigh the cost of reimplementing or wrapping existing software.
...
Performance overhead relative to native processes is modest for concurrent, message-oriented workloads and higher for tight numeric loops or latency-critical media processing.
The design therefore favours control-plane and interactive workloads over heavy data-plane computation unless carefully engineered NIFs are introduced.
...
In the long run, the architecture suggests a broader possibility: that the traditional Unix process, while historically foundational, need not remain the sole or even the primary abstraction for constructing reliable, concurrent, and inspectable systems on POSIX kernels.
